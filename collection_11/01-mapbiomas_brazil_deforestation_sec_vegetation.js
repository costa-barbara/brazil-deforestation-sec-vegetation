...
// assets
var assetInput = 'projects/mapbiomas-brazil/assets/LAND-COVER/COLLECTION-11/INTEGRATION/classification-ft';
var version = 'v00';
// var assetOutput = 'projects/ee-ipam/assets/MAPBIOMAS/LULC/DEFORESTATION/col11_def_sec_vegetation';
var assetOutput = 'projects/mapbiomas-brazil/assets/LAND-COVER/COLLECTION-11/DEFORESTATION/deforestation-secondary-vegetation';

// product versions
var inputVersion = '0-2-tra-1';
...
var classes = [
    ...
    [13, 2], // Outras formações nao florestais
...
var transitions_3 = transitions.slice(-3,-2);//.select('classification_2022');
var transitions_2 = transitions.slice(-2,-1);//.select('classification_2023');
var transitions_3 = transitions.slice(-1);//.select('classification_2024');

// ajusta ultimos anos da vegetação secundária não confirmada
transitions_2 = transitions_2.where(transitions_3.eq(1).and(transitions_2.eq(3)), 1);
transitions_3 = transitions_3.where(transitions_3.eq(1).and(transitions_3.eq(3)), 1);
transitions_3 = transitions_3.where(transitions_2.eq(1).and(transitions_3.eq(3)), 1);

// ajusta o último ano do desmatamento não confirmado em vegetação sencundária
transitions_3 = transitions_3.where(transitions_2.eq(3).and(transitions_3.eq(1)), 3);

transitions = transitions.addBands(transitions_2, null, true);
transitions = transitions.addBands(transitions_3, null, true);
...
gridNames.forEach(
    function (gridName) {
        ...
        Export.image.toAsset({
            ...
            description: 'def_sec_vegetation-' + version  + '-' + gridName,
            assetId: assetOutput + "/" + version  + '-' + gridName,
            pyramidingPolicy: {
                '.default': 'mode'
            },
