...
ASSET_TERRITORY = f'{PROJECT_TERRITORY_PATH}/TERRITORIES-OLD/LULC/{REGION_NAME.upper()}/COLLECTION9/dashboard/{TERRITORY_IMAGE_ID}'
REGION_MASK = ee.Image(ASSET_TERRITORY)

ASSET_TERRITORY_CORALS = f'projects/mapbiomas-territories/assets/TERRITORIES-STAGING/BRAZIL/WORKSPACE/COASTAL_MARINE_ZONE/COASTAL_MARINE_ZONE_v3'
REGION_MASK_CORALS = ee.Image().paint(
    featureCollection=ee.FeatureCollection(ASSET_TERRITORY_CORALS),
    color=1
)
# ================================
# VERSIONING
...
VERSION_INPUT_DEF_SEC_VEG        = '0-4-tra-1-2'
VERSION_PUBLIC_DEF_SEC_VEG       = 'v2'

VERSION_INPUT_DEF_SEC_VEG_ANN    = '0-4-tra-1-2'
VERSION_PUBLIC_DEF_SEC_VEG_ANN   = 'v2'

VERSION_INPUT_DEF_SEC_VEG_ACC    = '0-4-tra-1-2'
VERSION_PUBLIC_DEF_SEC_VEG_ACC   = 'v2'

VERSION_INPUT_DEF_FREQ           = '0-4-tra-1-2'
VERSION_PUBLIC_DEF_FREQ          = 'v2'

VERSION_INPUT_SEC_VEG_AGE        = '0-4-tra-1-2'
...
VERSION_INPUT_AGRICULTURE_IRRIGATION = '2'
VERSION_PUBLIC_AGRICULTURE_IRRIGATION = 'v3'
...
VERSION_INPUT_AGRICULTURE_SECOND_CROP      = '1'
VERSION_PUBLIC_AGRICULTURE_SECOND_CROP     = 'v1'

VERSION_INPUT_PASTURE_VIGOR      = '3'
VERSION_PUBLIC_PASTURE_VIGOR     = 'v3'

VERSION_INPUT_PASTURE_BIOMASS    = '1'
VERSION_PUBLIC_PASTURE_BIOMASS   = 'v2'
...
VERSION_INPUT_SOIL_TEXTURAL_CLASS = '1'
VERSION_PUBLIC_SOIL_TEXTURAL_CLASS = 'v1'

VERSION_INPUT_SOIL_CARBON        = '1'
VERSION_PUBLIC_SOIL_CARBON       = 'v1'

VERSION_INPUT_SOIL_STONINESS     = '1'
VERSION_PUBLIC_SOIL_STONINESS    = 'v1'
...
ASSET_PASTURE_VIGOR      = f'{ASSET_COLLECTION}/PASTURE/classification-vigor'
ASSET_PASTURE_BIOMASS    = f'{ASSET_COLLECTION}/PASTURE/classification-biomassa'
...
# Soil assets
ASSET_SOIL_SAND_FRACTION     = f'projects/mapbiomas-workspace/SOLOS/PRODUTOS_C03/mapbiomas_soil_collection3_sand'
ASSET_SOIL_CLAY_FRACTION     = f'projects/mapbiomas-workspace/SOLOS/PRODUTOS_C03/mapbiomas_soil_collection3_clay'
ASSET_SOIL_SILT_FRACTION     = f'projects/mapbiomas-workspace/SOLOS/PRODUTOS_C03/mapbiomas_soil_collection3_silt'
ASSET_SOIL_TEXTURAL_GROUP    = f'projects/mapbiomas-workspace/SOLOS/PRODUTOS_C03/mapbiomas_soil_collection3_textural_group'
ASSET_SOIL_TEXTURAL_SUBGROUP = f'projects/mapbiomas-workspace/SOLOS/PRODUTOS_C03/mapbiomas_soil_collection3_textural_subgroup'
ASSET_SOIL_TEXTURAL_CLASS    = f'projects/mapbiomas-workspace/SOLOS/PRODUTOS_C03/mapbiomas_soil_collection3_textural_class'
ASSET_SOIL_CARBON            = f'projects/mapbiomas-workspace/SOLOS/PRODUTOS_C03/mapbiomas_soil_collection3_soc_t_ha_000_030cm'
ASSET_SOIL_STONINESS         = f'projects/mapbiomas-workspace/SOLOS/PRODUTOS_C03/mapbiomas_soil_collection3_stoniness'
...
ASSET_AGRI_SECOND_CROP   = f'{ASSET_COLLECTION}/AGRICULTURE/classification_second_cycle'
...
PUBLIC_LULC_COLLECTION    = f'{PROJECT_PUBLIC_PATH}/{REGION_NAME}/lulc/collection{str(COLLECTION_ID).replace(".", "_")}'
PUBLIC_SOIL_COLLECTION    = f'{PROJECT_PUBLIC_PATH}/{REGION_NAME}/soil/collection{str(SOIL_COLLECTION_ID).replace(".", "_")}'
PUBLIC_WATER_COLLECTION   = f'{PROJECT_PUBLIC_PATH}/{REGION_NAME}/water/collection{WATER_COLLECTION_ID}'

PUBLIC_INTEGRATION            = f'{PUBLIC_LULC_COLLECTION}/mapbiomas_{REGION_NAME}_collection{str(COLLECTION_ID).replace(".", "_")}_coverage_{VERSION_PUBLIC_INTEGRATION}'
...
PUBLIC_DEF_SEC_VEG            = f'{PUBLIC_LULC_COLLECTION}/mapbiomas_{REGION_NAME}_collection{str(COLLECTION_ID).replace(".", "_")}_deforestation_secondary_vegetation_{VERSION_PUBLIC_DEF_SEC_VEG}'
PUBLIC_DEF_SEC_VEG_ANN        = f'{PUBLIC_LULC_COLLECTION}/mapbiomas_{REGION_NAME}_collection{str(COLLECTION_ID).replace(".", "_")}_deforestation_secondary_vegetation_{VERSION_PUBLIC_DEF_SEC_VEG_ANN}'
...
# outputs soil products
PUBLIC_SOIL_SAND_FRACTION     = f'{PUBLIC_SOIL_COLLECTION}/mapbiomas_{REGION_NAME}_collection{str(SOIL_COLLECTION_ID).replace(".", "")}_soil_sand_fraction_{VERSION_PUBLIC_SOIL_SAND_FRACTION}'
PUBLIC_SOIL_CLAY_FRACTION     = f'{PUBLIC_SOIL_COLLECTION}/mapbiomas_{REGION_NAME}_collection{str(SOIL_COLLECTION_ID).replace(".", "")}_soil_clay_fraction_{VERSION_PUBLIC_SOIL_CLAY_FRACTION}'
PUBLIC_SOIL_SILT_FRACTION     = f'{PUBLIC_SOIL_COLLECTION}/mapbiomas_{REGION_NAME}_collection{str(SOIL_COLLECTION_ID).replace(".", "")}_soil_silt_fraction_{VERSION_PUBLIC_SOIL_SILT_FRACTION}'
PUBLIC_SOIL_TEXTURAL_GROUP    = f'{PUBLIC_SOIL_COLLECTION}/mapbiomas_{REGION_NAME}_collection{str(SOIL_COLLECTION_ID).replace(".", "")}_soil_textural_group_{VERSION_PUBLIC_SOIL_TEXTURAL_GROUP}'
PUBLIC_SOIL_TEXTURAL_SUBGROUP = f'{PUBLIC_SOIL_COLLECTION}/mapbiomas_{REGION_NAME}_collection{str(SOIL_COLLECTION_ID).replace(".", "")}_soil_textural_subgroup_{VERSION_PUBLIC_SOIL_TEXTURAL_SUBGROUP}'
PUBLIC_SOIL_TEXTURAL_CLASS    = f'{PUBLIC_SOIL_COLLECTION}/mapbiomas_{REGION_NAME}_collection{str(SOIL_COLLECTION_ID).replace(".", "")}_soil_textural_class_{VERSION_PUBLIC_SOIL_TEXTURAL_CLASS}'
PUBLIC_SOIL_CARBON            = f'{PUBLIC_SOIL_COLLECTION}/mapbiomas_{REGION_NAME}_collection{str(SOIL_COLLECTION_ID).replace(".", "")}_soil_carbon_{VERSION_PUBLIC_SOIL_CARBON}'
PUBLIC_SOIL_STONINESS         = f'{PUBLIC_SOIL_COLLECTION}/mapbiomas_{REGION_NAME}_collection{str(SOIL_COLLECTION_ID).replace(".", "")}_soil_stoniness_{VERSION_PUBLIC_SOIL_STONINESS}'
...
PUBLIC_AGRICULTURE_SECOND_CROP       = f'{PUBLIC_LULC_COLLECTION}/mapbiomas_{REGION_NAME}_collection{COLLECTION_ID}_agriculture_second_crop_{VERSION_PUBLIC_AGRICULTURE_SECOND_CROP}'
...
YEARS_DEF_SEC_VEG     = list(range(1987, 2025))
YEARS_MINING_SUBSTANCES = list(range(1985, 2024))
YEARS_PASTURE_VIGOR   = list(range(2000, 2025))
YEARS_PASTURE_BIOMASS = list(range(2000, 2025))
...
YEARS_AGRICULTURE_SECOND_CROP  = list(range(2000, 2025)) 
YEARS_DEG_EDGE_SIZE   = list(range(1985, 2025))
YEARS_DEG_PATCH_SIZE  = list(range(1985, 2025))
YEARS_SOIL_CARBON     = list(range(1985, 2025))
...
RUN_EXPORT_MINING_ANNUAL          = False
...
def export_public_pasture_vigor():
    ...
    for year in YEARS_PASTURE_VIGOR:
        asset_id = f'{ASSET_PASTURE_VIGOR}/{year}-{VERSION_INPUT_PASTURE_VIGOR}'
        try:
            image = ee.Image(asset_id).rename(f'classification_{year}')
            band_images.append(image)
            print(f'✅ Loaded: {asset_id}')
    ...
    properties = {
        "module": "Pasture",
        "submodule": "Vigor condition",
        "variable": "Vigor condition",
        "format": "classification_multiband_image",
        "band_format": "classification_{year}",
        "data_type": "annual",
        "version": VERSION_PUBLIC_PASTURE_VIGOR,
    }

    return export_image_to_asset(image, PUBLIC_PASTURE_VIGOR, version=VERSION_PUBLIC_PASTURE_VIGOR, additional_properties=properties)


def export_public_pasture_biomass():
    """
    Exports the pasture biomass image for the region.
    """
    band_images = []

    for year in YEARS_PASTURE_BIOMASS:
        asset_id = f'{ASSET_PASTURE_BIOMASS}/{year}-{VERSION_INPUT_PASTURE_BIOMASS}'
        try:
            image = ee.Image(asset_id).rename(f'biomass_{year}')
            band_images.append(image)
            print(f'✅ Loaded: {asset_id}')
        except Exception as e:
            print(f'❌ Failed to load {asset_id}: {e}')
    ...
    properties = {
        ...
        "variable": "Biomass",
        "format": "classification_multiband_image",
        "band_format": "biomass_{year}",
        "data_type": "annual",
        "unit": "ton/ha",
        "version": VERSION_PUBLIC_PASTURE_BIOMASS,
    }

    return export_image_to_asset(image, PUBLIC_PASTURE_BIOMASS, version=VERSION_PUBLIC_PASTURE_BIOMASS, additional_properties=properties)
...
def export_public_soil_sand_fraction():
    ...
    try:
        ...
        ensure_asset_exists(PUBLIC_SOIL_SAND_FRACTION)
...
def export_public_soil_clay_fraction():
    ...
    try:
        ...
        ensure_asset_exists(PUBLIC_SOIL_CLAY_FRACTION)
        set_asset_properties(PUBLIC_SOIL_CLAY_FRACTION, collection_properties)

        tasks = []
        for depth_start, depth_end in SOIL_DEPTHS:
            original_band = f'clay_{depth_start}_{depth_end}cm' #clay_000_010cm
...
def export_public_soil_silt_fraction():
    ...
    try:
        ...
        ensure_asset_exists(PUBLIC_SOIL_SILT_FRACTION)
...
def export_public_soil_textural_subgroup():
    ...
    try:
        ...
        ensure_asset_exists(PUBLIC_SOIL_TEXTURAL_SUBGROUP)
        set_asset_properties(PUBLIC_SOIL_TEXTURAL_SUBGROUP, collection_properties)

        tasks = []
        for depth_start, depth_end in SOIL_DEPTHS_TEXTURAL:
            original_band = f'textural_subgroup_{depth_start}_{depth_end}cm'
...
def export_public_soil_textural_class():
    ...
    try:
        ...
        ensure_asset_exists(PUBLIC_SOIL_TEXTURAL_CLASS)
        set_asset_properties(PUBLIC_SOIL_TEXTURAL_CLASS, collection_properties)

        tasks = []
        for depth_start, depth_end in SOIL_DEPTHS_TEXTURAL:
            original_band = f'textural_class_{depth_start}_{depth_end}cm'
...
def export_public_soil_carbon():
    ...
    try:
        ...
        original_bands = [f'carbon_{year}' for year in YEARS_SOIL_CARBON]
...
def export_public_soil_stoniness():
    """
    Exports the soil stoniness image for the region.
    """
    try:
        image = ee.Image(ASSET_SOIL_STONINESS)

        collection_properties = {
            "module": "Soil",
            "submodule": "Depth",
            "variable": "Stoniness",
            "band_format": "soil_depth_stoniness",
            "version": VERSION_PUBLIC_SOIL_STONINESS,
            "data_type": "constant",
            "format": "stratified_continuous_singleband_image",
            "stratification": "volume",
            "depths": json.dumps(SOIL_DEPTHS_STONINESS_VOLUME),
            "unit": "cm",
        }

        ensure_asset_exists(PUBLIC_SOIL_STONINESS)
        set_asset_properties(PUBLIC_SOIL_STONINESS, collection_properties)
        tasks = []
        for volume in SOIL_DEPTHS_STONINESS_VOLUME:
            original_band = f'soildepth_stone{volume}vol_cm'

            asset_name = f'soil_depth_stoniness_{volume}_{VERSION_PUBLIC_SOIL_STONINESS}'

            image_band = image.select([original_band], ['soil_depth_stoniness'])
            ...
            tasks.append(export_image_to_asset(image_band, 
                                                f'{PUBLIC_SOIL_STONINESS}/{asset_name}', 
                                                additional_properties=properties))
    except Exception as e:
        print(f'❌ Failed to export: {e}')
...
def export_public_agriculture_second_crop():
    """
    Exports the agriculture second crop image for the region.
    This function filters the collection by version and exports the minimum image.
    """
    try:
        image = (
            ee.ImageCollection(ASSET_AGRI_SECOND_CROP)
                 .filter(ee.Filter.eq('version', VERSION_INPUT_AGRICULTURE_SECOND_CROP))
                 ).toBands()

        image = image.rename([f'classification_{year}' for year in YEARS_AGRICULTURE_SECOND_CROP])
    ...
    properties = {
        "module": "Agriculture",  
        "submodule": "Agriculture",  
        "variable": "Second Crop Presence",  
        "format": "classification_multiband_image",
        "band_format": "classification_{year}",
        "data_type": "annual",
        "version": VERSION_PUBLIC_AGRICULTURE_SECOND_CROP,
    }

    return export_image_to_asset(image, 
                                 PUBLIC_AGRICULTURE_SECOND_CROP, 
                                 scale=EXPORT_SCALE,
                                 additional_properties=properties)
...
def export_public_coral_reefs():
    ...
    return export_image_to_asset(image, 
                                 PUBLIC_CORAL_REEFS, 
                                 additional_properties=properties,
                                 region_mask=REGION_MASK_CORALS)
...
if RUN_EXPORT_INTEGRATION:
    export_tasks.append(export_collection_product(
        ...
        additional_properties={
            ...
            "band_format": "classification_{year}",
            "data_type": "annual",
            "version": VERSION_PUBLIC_INTEGRATION,
        },
    ))
...
if RUN_EXPORT_DEF_SEC_VEG:
    export_tasks.append(export_collection_product(
        ...
        additional_properties={
            "module": "Deforestation/Secondary Vegetation",
            "submodule": "Deforestation/Secondary Vegetation Annual",
            "format": "classification_multiband_image",
            "band_format": "classification_{year}",
            "data_type": "annual",
...
if RUN_EXPORT_SOIL_STONINESS:
    export_tasks.extend(export_public_soil_stoniness())

if RUN_EXPORT_AGRI_NUM_CYCLES:
    export_tasks.append(export_public_agriculture_number_cycles())

if RUN_EXPORT_AGRI_SECOND_CROP:
    export_tasks.append(export_public_agriculture_second_crop())
