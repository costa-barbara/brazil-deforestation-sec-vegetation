...
def wait_until_tasks_finish(export_tasks=[], polling_interval=60):
    ...
    Continues monitoring even if some tasks fail. Prints a summary report on each
    polling cycle: total tasks, breakdown by status, active (RUNNING) tasks listed
    by name, and FAILED tasks with error messages.

    Uses GEE task descriptions to query fresh status from the API instead of relying
    on potentially stale task object references.
    """
    if not export_tasks:
    ...
    print(f"\n⏳ Monitoring {total} task(s)...")

    # Extract task descriptions for querying fresh status from GEE
    target_descriptions = set()
    for task in export_tasks:
        try:
            status = task.status()
            desc = status.get('description', '')
            if desc:
                target_descriptions.add(desc)
        except:
            pass
    ...
    while True:
        counts = Counter({'COMPLETED': 0, 'RUNNING': 0, 'READY': 0, 'FAILED': 0, 'CANCELLED': 0})
        running_names = []
        all_done = True

        # Query fresh task status from GEE instead of using stale task objects
        all_gee_tasks = ee.batch.Task.list()
        found_tasks = {}

        for gee_task in all_gee_tasks:
            task_desc = gee_task.config.get('description', '')
            if task_desc in target_descriptions:
                found_tasks[task_desc] = gee_task

        # Check status of all tracked tasks
        for desc in target_descriptions:
            if desc in found_tasks:
                gee_task = found_tasks[desc]
                state = gee_task.state
                counts[state] += 1

                if state in ('READY', 'RUNNING'):
                    all_done = False
                    if state == 'RUNNING':
                        running_names.append(desc)
                elif state == 'FAILED' and desc not in completed:
                    error_message = gee_task.config.get('error_message', 'No error message provided.')
                    failed_tasks.append((desc, error_message))
                    completed.add(desc)
                elif state in ('COMPLETED', 'CANCELLED'):
                    completed.add(desc)
            else:
                # Task not found in GEE; assume it completed or was cancelled
        ...
        if all_done and len(completed) == len(target_descriptions):
            total_time = str(timedelta(seconds=int(time.time() - start_time)))
            print(f"\n✅ All {total} tasks finished. Total time: {total_time}")
            if failed_tasks:
